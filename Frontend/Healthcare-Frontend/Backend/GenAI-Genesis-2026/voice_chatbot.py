...
# IBM Watson — commented out where replaced by ElevenLabs
...
IBM_REQUIRED = not ELEVENLABS_API_KEY
if IBM_REQUIRED and not all([STT_API_KEY, STT_URL, TTS_API_KEY, TTS_URL]):
    log.error(
        "One or more IBM Watson credentials are missing from the .env file.\n"
        "Required keys: STT_API_KEY, STT_URL, TTS_API_KEY, TTS_URL\n"
        "Or set ELEVENLABS_API_KEY to use ElevenLabs instead."
    )
    sys.exit(1)
...
# Other voice: en-US_EmmaExpressive
...
def _parse_int_env(var_name: str, default: int | None = None) -> int | None:
    ...
    try:
        ...
        log.warning("%s=%r is invalid; expected integer. Using default %r.", var_name, raw, default)
        return default


# Optional override. Leave unset to auto-select a working input device.
MIC_DEVICE_INDEX = _parse_int_env("MIC_DEVICE_INDEX", None)
...
def speak(tts_client: TextToSpeechV1 | None, text: str, tts_active: threading.Event) -> None:
    ...
    try:
        if tts_client is None:
            # ElevenLabs TTS (replaces IBM when tts_client is None)
            from tts_elevenlabs import synthesize as elevenlabs_synthesize
            import re
            plain_text = re.sub(r"<[^>]+>", "", wrap_in_ssml(text)) or text
            audio_bytes = elevenlabs_synthesize(plain_text, output_format="wav_44100")
            wav_bytes = io.BytesIO(audio_bytes)
        else:
            # IBM Watson TTS (legacy)
...
class MicrophoneStream:
    ...
    def __init__(
        ...
        if MIC_DEVICE_INDEX is not None:
            candidate_indices.append(MIC_DEVICE_INDEX)
            log.info("Microphone: trying override device index %d", MIC_DEVICE_INDEX)

        try:
            info = pa.get_default_input_device_info()
            default_idx = int(info["index"])
            if default_idx not in candidate_indices:
                candidate_indices.append(default_idx)
        except OSError:
            log.warning("Could not query default input device")

        if None not in candidate_indices:
        ...
        for idx in candidate_indices:
            try:
                ...
                if idx is None:
                    log.info("Microphone: opened using PyAudio default input device")
                else:
                    try:
                        dev_info = pa.get_device_info_by_index(idx)
                        log.info("Microphone: opened %s (device %d)", dev_info.get("name", "unknown"), idx)
                    except Exception:
                        log.info("Microphone: opened device %d", idx)
                break
            except OSError as exc:
                last_error = exc
                if idx is None:
                    log.warning("Microphone open attempt failed using PyAudio default: %s", exc)
                else:
                    log.warning("Microphone open attempt failed for device %d: %s", idx, exc)

        if self._stream is None:
            log.error(
                "Cannot open microphone after trying devices %s: %s\n"
                "Set MIC_DEVICE_INDEX in .env to a valid input device index from the startup list.",
                candidate_indices,
                last_error,
            )
            raise last_error or OSError("No usable microphone input device found")
