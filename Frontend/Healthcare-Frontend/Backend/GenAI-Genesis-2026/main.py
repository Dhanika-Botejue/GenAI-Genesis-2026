...
# IBM Watson TTS/STT — commented out, replaced with ElevenLabs
...
import pyaudio
# from ibm_watson.websocket import AudioSource  # IBM — replaced with ElevenLabs
...
# IBM Watson transcript format — kept for reference if needed
...
def _synthesize_prompt_to_public_wav(text: str) -> str:
    """Synthesize TTS using ElevenLabs. Returns public URL for Twilio <Play>."""
    ...
    # ElevenLabs TTS — plain text (no SSML; ElevenLabs uses different format)
    plain_text = re.sub(r"<[^>]+>", "", wrap_in_ssml(text)) or text
    audio_bytes = elevenlabs_synthesize(plain_text, output_format="mp3_44100_128")

    filename = f"prompt_{int(time.time() * 1000)}_{uuid.uuid4().hex[:8]}.mp3"
...
def _prompt_node(text: str) -> str:
    """
    Use ElevenLabs TTS for Twilio calls.
    If ElevenLabs fails (e.g., credentials), fall back to Twilio <Say>.
    """
    try:
...
def _transcribe_twilio_recording(recording_url: str) -> str:
    """Transcribe Twilio recording using ElevenLabs STT."""
    from stt_elevenlabs import transcribe_audio_sync

    wav_bytes = _fetch_twilio_recording_wav(recording_url)
    return transcribe_audio_sync(wav_bytes, content_type="audio/wav")
...
# ── IBM Watson clients (commented out — replaced with ElevenLabs) ───────────────
...
def start_voice_chatbot() -> None:
    ...
    # ElevenLabs does not support streaming STT; local voice chatbot requires IBM for mic input.
    # When using ElevenLabs, skip the voice chatbot (Twilio calls and /transcribe still work).
    if os.getenv("ELEVENLABS_API_KEY") or os.getenv("XI_API_KEY"):
        log.info("ElevenLabs configured — skipping local voice chatbot (no streaming STT).")
        return

    try:
        from ibm_watson import SpeechToTextV1, TextToSpeechV1
        from ibm_cloud_sdk_core.authenticators import IAMAuthenticator
        stt = SpeechToTextV1(authenticator=IAMAuthenticator(os.getenv("STT_API_KEY")))
        stt.set_service_url(os.getenv("STT_URL"))
        tts = TextToSpeechV1(authenticator=IAMAuthenticator(os.getenv("TTS_API_KEY")))
        tts.set_service_url(os.getenv("TTS_URL"))
        stt_client, tts_client = stt, tts
    except Exception as exc:
        log.warning("IBM credentials missing or invalid — voice chatbot disabled: %s", exc)
        return
    ...
    # STT WebSocket loop (IBM only — ElevenLabs has no streaming STT)
...
async def test_tts():
    """
    Returns a short ElevenLabs TTS clip. Use to verify the backend uses ElevenLabs, not IBM.
    """
    from tts_elevenlabs import synthesize
    text = "This is ElevenLabs text to speech. If you hear this, the backend is using ElevenLabs correctly."
    audio_bytes = synthesize(text, output_format="mp3_44100_128")
...
async def transcribe(file: UploadFile = File(...)):
    ...
    Uses ElevenLabs STT for transcription.
    """
    temp_path = None
    ...
    try:
        ...
        # STT — ElevenLabs (replaces IBM Watson)
        from stt_elevenlabs import transcribe_audio_sync
        with open(temp_path, "rb") as audio_file:
            audio_bytes = audio_file.read()
        content_type = f"audio/{suffix.lstrip('.')}" if suffix else "audio/wav"
        transcript = transcribe_audio_sync(audio_bytes, content_type=content_type)
        ...
        # TTS — speak the reply back (ElevenLabs; tts_client=None uses ElevenLabs in speak)
        tts_active = _voice_state.get("tts_active") or threading.Event()
        if llm_result.get("tts_reply"):
            threading.Thread(
                target=speak,
                args=(None, llm_result["tts_reply"], tts_active),
                daemon=True,
            ).start()
...
async def twilio_recording(
    ...
    """Twilio webhook: transcribe resident answer with ElevenLabs STT and continue flow."""
    try:
        if session_id:
            state = _twilio_calls.get(session_id)
            if not state:
