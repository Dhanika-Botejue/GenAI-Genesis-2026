...
def _build_record_twiml(prompt_text: str, action_path: str) -> str:
    """
    Uses <Gather input="speech"> for low-latency response detection.
    Twilio ends input ~0.5s after the patient stops talking (vs a fixed silence
    timeout) and returns the transcript inline, eliminating the recording
    download + IBM STT round-trip entirely.
    """
    action_url = _public_url(action_path)
    try:
        prompt_audio_url = _synthesize_prompt_to_public_wav(prompt_text)
        prompt_node = f"<Play>{html.escape(prompt_audio_url)}</Play>"
    except Exception:
        prompt_node = f'<Say voice="alice">{html.escape(prompt_text)}</Say>'

    speech_timeout = os.getenv("TWILIO_SPEECH_TIMEOUT", "1")
    return (
        '<?xml version="1.0" encoding="UTF-8"?>'
        "<Response>"
        f'<Gather input="speech" action="{html.escape(action_url, quote=True)}" '
...
def _normalize_session_doc(doc: dict[str, Any], live_state: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # Prefer the persisted answers array (written by _complete_session),
    # then live in-memory answers, then parsed transcript as last resort.
    db_answers: list[dict[str, str]] = []
    for answer in doc.get("answers") or []:
        ...
        answer_text = str(answer.get("answer") or "").strip()
        if question and answer_text:
            db_answers.append({"question": question, "answer": answer_text})

    best_answers = live_answers or db_answers or parsed_answers

    # Questions: prefer persisted questions_asked, then call_config, then inferred from answers.
    db_questions = [str(q).strip() for q in (doc.get("questions_asked") or []) if str(q).strip()]
    config_questions = [str(q).strip() for q in (call_config.get("questions") or []) if str(q).strip()]
    normalized_questions = (
        db_questions
        or config_questions
        or [a["question"] for a in best_answers if a.get("question")]
    )
    ...
    normalized = {
        ...
        "greeting_notes": str(
            live_state.get("greeting_notes")
            or doc.get("greeting_notes")
            or parsed_greeting_notes
            or ""
        ).strip(),
...
def _generate_greeting_reply(first_name: str, patient_said: str) -> str:
    """
    Uses the LLM to generate a warm, natural reply to whatever the patient says
    during the greeting turn, then transitions into the questions.
    Falls back to a safe static reply on any error.
    """
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        return "That is great to hear! I have a few questions for you today."

    prompt = (
        f"You are a warm, caring AI health assistant on a phone call with a patient named {first_name}. "
        f"You just said hello and asked how they are. They responded:\n\"{patient_said}\"\n\n"
        "Generate a SHORT, natural reply (1-2 sentences, under 20 words total) that:\n"
        ...
        "- If they said something casual or unrelated → respond naturally and kindly\n"
        "- ALWAYS end with a transition like 'I have a few questions for you today.' "
        "or 'Let me go ahead with a couple of questions.'\n\n"
        "CRITICAL: Keep it under 20 words. Be natural, not robotic. Return ONLY the reply text, no quotes."
    )
    ...
    try:
        ...
        return reply or "That is lovely to hear! I have a few questions for you today."
    except Exception as exc:
        log.warning("Greeting reply generation failed: %s", exc)
        return "That is great to hear! I have a few questions for you today."
...
def _append_session_transcript(session_id: str, line: str) -> None:
    ...
    # Use $push on a lines array (atomic) rather than read-modify-write which
    # can silently drop data when the session doc doesn't exist yet or on concurrent updates.
...
def _complete_session(session_id: str, call_status: str = "completed") -> None:
    """Mark session complete and flush all in-memory call state into MongoDB."""
    call_state = _twilio_calls.get(session_id) or {}
    answers = call_state.get("answers") or []
    history = call_state.get("history") or []
    greeting_notes = call_state.get("greeting_notes") or ""
    questions = call_state.get("questions") or []

    # Build raw_transcript from transcript_lines (if stored as array) or reconstruct from answers
    now = datetime.utcnow()
    ...
    if not _DB_AVAILABLE:
        ...
        if session is not None:
            ...
            if greeting_notes:
                session["greeting_notes"] = greeting_notes
        return

    db = db_service.get_db()

    # Rebuild raw_transcript from the atomic transcript_lines array (if present)
    # so the text field stays in sync for any code that reads it directly.
    existing = db.ai_sessions.find_one({"_id": session_id}, {"transcript_lines": 1, "raw_transcript": 1})
    updates: dict = {
        "status": "completed",
    ...
    if existing:
        lines = existing.get("transcript_lines") or []
        if lines:
            updates["raw_transcript"] = "".join(lines)

    # Persist structured call data so history queries don't rely on transcript parsing.
    if answers:
        updates["answers"] = answers
    if questions:
        updates["questions_asked"] = questions
    if greeting_notes:
        updates["greeting_notes"] = greeting_notes
    if history:
...
async def initiate_patient_call(payload: InitiatePatientCallRequest):
    ...
    # Pre-warm the TTS audio cache so Twilio's first webhook returns instantly.
    # Do this before Twilio even dials — synthesis takes ~0.5-1s so the audio
    # is ready the moment the call connects.
    def _prewarm():
        try:
            _synthesize_prompt_to_public_wav(greeting)
        except Exception:
            pass
        for q in questions:
            try:
                _synthesize_prompt_to_public_wav(q)
            except Exception:
                pass
...
async def twilio_recording(
    ...
    try:
        if session_id:
            ...
            # Prefer inline SpeechResult from <Gather> (fast path).
            # Fall back to downloading + IBM STT if only a RecordingUrl is present.
            if SpeechResult is not None:
                ...
                # No speech and no recording — retry the same prompt
                if phase == "greeting":
            ...
            if not transcript:
                if phase == "greeting":
                    retry_prompt = (
                        "I could not hear you clearly. Please speak now. "
            ...
            if phase == "greeting":
                ...
                first_name = state.get("patient_name", "there")
                ack = _generate_greeting_reply(first_name, transcript)
