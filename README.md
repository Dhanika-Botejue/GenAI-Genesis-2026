...
- [IBM Watson Speech to Text](https://www.ibm.com/products/speech-to-text) fits the call workflow because recorded answers can be transcribed after each Twilio webhook step and stored as structured follow-up data, while aligning the demo with the tighter security and privacy expectations that come with PIPEDA-sensitive healthcare use cases.
- [IBM Watson Text to Speech](https://www.ibm.com/products/text-to-speech) fits the prompt workflow because generated clips can be cached in [audio/](./audio/) and replayed across repeated call sessions, and the same IBM stack keeps the voice layer consistent with a security-first, PIPEDA-aware deployment story.
...
- [IBM Watson Speech to Text](https://www.ibm.com/products/speech-to-text) fits the phone workflow well because the backend already stores and replays audio as regular files and webhook-driven assets.
...
- [IBM Watson Text to Speech](https://www.ibm.com/products/text-to-speech) is a practical fit for prompt generation because the synthesized clips can be cached in [audio/](./audio/) and served back to Twilio as regular HTTP assets.
